...
- **Статичная тема + динамический акцент**: `base16.json` использует палитру zenburn. `colors.json` отвечает за градиентные фоны, текст и акцент, извлекаемый `wallust` из обоев для подсветки элементов.

## -- Дерево рабочей части проекта и назначение модулей --:
```
.
├── shell.qml                 # Точка входа Quickshell. Регистрирует и позиционирует модули.
├── colors.json               # Основная тема интерфейса.
├── base16.json               # Дополнительная тема интерфейса.
├── bar/                      # панель.
...
**Метрики**: CPU idle ~5–7% (Go subscribe) против 35–45% (bash polling). Бинарники собраны статически, вес логики ~10-11 МБ.
...
2. **Смена темы**: Отредактировать конфиг `wallust` (можно ещё переписать `base16.json`, но он почти не влияет на визуальную часть *JES*) → перегенерировать палитру.
...
3. откройте shell.qml
4. в import впишите папку модуля с плагином
5. ниже, где другие подключения, сделайте ctrl c ctrl v названия модуля + добавьте {} через пробел от названия
```
### Включение и выключение работает через комментирования строки с "Плагин {}"
