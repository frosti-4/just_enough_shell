# structure_rus.md
Архитектура и модульная структура проекта
...
2. **Сбор данных**: 
   - Go-бинарники (`sys_info`, `music`, `Cava-internal`, `timed`, `cal`) берут на себя логику с большими объёмами данных, которые надо обработать (искл: timed, он был сделан, когда изучался go).
   - Bash-скрипты (`brightness.sh`, `vol.sh`, `workspace-*.sh`) являются оснвной логикой, сделано для переносимсоти системы и читаемости.
...
- Ассеты (шейдеры, искохдники go, пустые скрипты-болванки для подключения другого тайлинга): см. `for-quickshell/`
