...
- **Статичная тема + динамический акцент**: `base16.json` использует палитру zenburn. `colors.json` отвечает за градиентные фоны, текст и акцентные цвета, всё извлекается из обоев благодарая `matugen`.
